"""
Standalone render script for Certificate Page 2 (No Header/Footer)
...
def render_certificate_page2_no_header_footer(data, output_path=None, template_dir=None):
    """
    Render certificate page 2 HTML from template and data (no header/footer version).
    ...
    Note: Page 2 requires additional data fields for calibration results:
        - repeatability_data: List of dicts with repeatability test data
        - reproducability_data: List of dicts with reproducability test data
        - geometric_data: List of dicts with geometric effect test data
    """
    ...
    template = env.get_template('certificate_page2_no_header_footer.html')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Usage: python render_certificate_page2_no_header_footer.py <inward_id> [api_url]")
            sys.exit(1)
    else:
    ...
    # Render and save
    output_file = 'certificate_page2_no_header_footer_rendered.html'
    render_certificate_page2_no_header_footer(data, output_path=output_file)
    
    print(f"Rendered certificate page 2 (no header/footer) -> {output_file}")
