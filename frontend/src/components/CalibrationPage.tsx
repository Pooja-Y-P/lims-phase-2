...
interface HTWJobResponse {
  ...
  // Technical Fields matching Python Schema
...
const CalibrationPage: React.FC = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      try {
        ...
        // Calculate Default Resolution (Reference Logic)
        ...
        // C. Check for Existing Job
        const checkJobUrl = `/htw-jobs/?inward_eqp_id=${equipmentId}`;
        console.log(`🔍 [DEBUG] Checking for existing job: ${checkJobUrl}`);

        let existingJob: HTWJobResponse | null = null;
        try {
            const jobRes = await api.get<any>(checkJobUrl);
            ...
            console.warn("No existing job found.");
        }

        if (existingJob) {
            console.log("✅ [DEBUG] Found Saved Job. Filling Data...", existingJob);
            
            setJobId(existingJob.job_id);
            setIsValidated(true);

            // D. Populate Form with SAVED JOB Data
            setDeviceDetails({
                ...
                // Load saved resolution or fallback to default
            ...
            console.log("ℹ️ [DEBUG] No Job Found. Using Defaults.");

            // E. Populate Form with INWARD Data + DEFAULT Resolution
            setDeviceDetails(prev => ({
                ...
                // Auto-fill logic from reference
  ...
  // 3. Auto-populate Range Unit (Only if NOT validated)
  useEffect(() => {
  ...
  // UPDATED: Logic exactly from reference code
  const handleResolutionUnitChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setDeviceDetails(prev => ({
        ...
        // If a match is found, use its pressure value. Otherwise, keep existing.
  ...
  const handleSaveWorksheet = async () => {
    ...
    try {
      ...
      if (response.status === 200 || response.data.status === "success") {
        alert("Worksheet Saved & Standards Auto-Selected Successfully!");
  ...
  if (error) {
    return (
      <div className="bg-white h-screen flex flex-col justify-center items-center p-8 text-center">
        ...
        <button onClick={handleBack} className="px-4 py-2 bg-gray-800 text-white rounded-lg">Back</button>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 bg-white">
        ...
        {/* Device Specifications Section Title */}
        <div className="mb-6">
        ...
        {/* --- DEVICE FORM GRID --- */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-12 gap-y-5 pb-8">
            
            {/* 1. Calibration Date */}
            <div className={wrapperClass}>
                <label className={labelClass}><span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> Calibration Date</span></label>
                <input
                    type="date"
                    value={deviceDetails.calibrationDate}
                    onChange={(e) => handleDeviceDetailChange('calibrationDate', e.target.value)}
            ...
            <div className={wrapperClass}>
                ...
                <input
                    ...
                    onChange={(e) => handleDeviceDetailChange('materialNomenclature', e.target.value)}
                    className={editableInput}
                />
            </div>
            ...
            <div className={wrapperClass}>
                ...
                <input
                    ...
                    onChange={(e) => handleDeviceDetailChange('make', e.target.value)}
                    className={editableInput}
                />
            </div>
            ...
            <div className={wrapperClass}>
                ...
                <input
                    ...
                    onChange={(e) => handleDeviceDetailChange('model', e.target.value)}
                    className={editableInput}
                />
            </div>
            ...
            <div className={wrapperClass}>
                ...
                <select 
                    ...
                    onChange={(e) => handleDeviceDetailChange('type', e.target.value)}
                    className={editableInput}
                >
            ...
            <div className={wrapperClass}>
                ...
                <input 
                    ...
                    onChange={(e) => handleDeviceDetailChange('classification', e.target.value)}
                    className={editableInput}
                />
            </div>
            ...
            <div className={wrapperClass}>
                ...
                <input
                    ...
                    onChange={(e) => handleDeviceDetailChange('serialNo', e.target.value)}
                    className={editableInput}
                />
            </div>

            {/* 8. Range (Grouped Input) */}
            <div className={wrapperClass}>
                ...
                <div className={groupContainer}>
                    <input 
                        ...
                        onChange={(e) => handleDeviceDetailChange('range', e.target.value)}
                        placeholder="Min - Max"
                        className={groupLeft}
                    />
                    <input
                        ...
                        onChange={(e) => handleDeviceDetailChange('rangeUnit', e.target.value)}
                        placeholder="Unit"
            ...
            {/* 9. Resolution (Grouped Input) - UPDATED TO MATCH REFERENCE EXACTLY */}
            <div className={wrapperClass}>
                ...
                <div className={groupContainer}>
                    <input 
                        ...
                        // IMPORTANT: Changed to readOnly to behave like reference
                        readOnly 
                    ...
                    >
                         ...
                         {allResolutions.map((res, index) => (
                            <option key={`${res.unit}-${index}`} value={res.unit}>{res.unit}</option>
                        ))}
                    </select>
                </div>
            </div>
            ...
            <div className={wrapperClass}>
                ...
                <select 
                    ...
                    onChange={(e) => handleDeviceDetailChange('calibrationMode', e.target.value)}
                    className={editableInput}
                >
        ...
        {/* Validation Action - HIDDEN if isValidated is true */}
        {!isValidated && (
            <div className="flex justify-end border-t border-gray-100 pt-6 pb-6">
                <button
                    onClick={handleValidateAndCreate}
                    disabled={validating}
                    className="px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-70 flex items-center gap-2"
                >
        ...
        {/* --- MASTER STANDARDS (Table Layout) - SHOWN if isValidated is true --- */}
        {isValidated && (
                                        ...
                                        <td key={stdKey} className="px-4 py-2">
                                            <div className={groupContainer}>
                                                <input
                                                    ...
                                                    className={groupLeft.replace("bg-gray-50", "bg-white")} // Make editable
                                                    placeholder="Val"
                                                />
                                                <input
                                                    type="text"
                                                    ...
                                                    className={groupRight.replace("bg-gray-100", "bg-white")} // Make editable
                                                    placeholder="Unit"
                                                />
                                            </div>
                                        </td>
      ...
      <div className="flex-none px-8 py-5 border-t border-gray-100 flex justify-end gap-4 bg-gray-50">
        <button 
            onClick={handleBack}
            className="px-5 py-2 text-sm text-gray-700 font-medium hover:bg-gray-200 rounded-lg transition-colors border border-gray-300 bg-white"
        >
            Cancel
        </button>
        {isValidated ? (
            <button 
                ...
                className="px-5 py-2 text-sm bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors shadow-sm flex items-center gap-2"
            >
                <Save className="h-4 w-4" />
                Save Worksheet (Auto-Select)
            </button>
        ) : (
             <button 
