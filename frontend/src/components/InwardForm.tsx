...
// FIX: Aliased the imported 'InwardForm' type to 'InwardFormType' to resolve the name collision.
...
export const InwardForm: React.FC = () => {
  ...
  // FIX: Updated to use the aliased type 'InwardFormType'.
  ...
  const initializeForm = async () => {
    try {
      ...
      // FIX: Updated the type annotation for `prev`.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        commitUsedSRFNo(response.data.srf_no);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70" onClick={() => setShowEmailModal(false)}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg m-4 p-8 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-gray-600 mb-6">
          Inward SRF <strong>{lastSavedSrfNo}</strong> is saved. Choose an option to send the report.
        </p>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6 bg-gray-50 rounded-lg border">
            <div>
              ...
              <input type="date" name="date" value={formData.date} onChange={handleFormChange} required className="w-full px-4 py-2 border rounded-lg" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Customer DC Date *</label>
              <input type="date" name="customer_dc_date" value={formData.customer_dc_date} onChange={handleFormChange} required className="w-full px-4 py-2 border rounded-lg" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Receiver *</label>
              <input type="text" name="receiver" value={formData.receiver} onChange={handleFormChange} required placeholder="Enter receiver username" className="w-full px-4 py-2 border rounded-lg" />
            </div>
            <div className="md:col-span-1">
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                SRF No <span className="text-gray-500">(Auto)</span>
              </label>
              <input type="text" value={formData.srf_no} disabled className="w-full px-4 py-2 bg-gray-200 rounded-lg" />
            </div>
            <div className="md:col-span-2 lg:col-span-3">
              <label className="block text-sm font-semibold text-gray-700 mb-2">Company Name & Address *</label>
              <input type="text" name="customer_details" value={formData.customer_details} onChange={handleFormChange} required placeholder="Enter company name and address" className="w-full px-4 py-2 border rounded-lg" />
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
                ...
                {equipmentList.map((equipment, index) => (
                  <tr key={index} className="hover:bg-slate-50 group">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {equipment.photos.map((photo: File, pIndex: number) => (
                              <div key={pIndex} className="flex items-center bg-gray-100 p-1 rounded text-xs" title={photo.name}>
                                ...
                                <button type="button" onClick={() => handleRemovePhoto(index, pIndex)} className="ml-1 text-red-500 hover:text-red-700"><X size={14} /></button>
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    <td className="p-2 align-middle text-center">
                      {loadingCodes[index] ? <Loader2 className="animate-spin text-slate-400" /> : equipment.barcode_image ? (
                        <button type="button" onClick={() => setViewingCodesFor(equipment)} className="p-2 text-slate-500 hover:bg-indigo-100 hover:text-indigo-600 rounded-full" title="View Codes"><ScanLine size={16} /></button>
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className='flex gap-4'>
              <button type="button" onClick={() => setShowStickerSheet(true)} className="flex items-center space-x-2 bg-slate-600 text-white px-4 py-2 rounded-lg hover:bg-slate-700 font-semibold"><Barcode size={20} /><span>Print Stickers</span></button>
              <button type="button" onClick={addEquipmentRow} className="flex items-center space-x-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-semibold"><Plus size={20} /><span>Add Another Equipment</span></button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md m-4 p-6 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-6 text-center">
              <div>
                ...
                {viewingCodesFor.barcode_image ? (<div className="bg-white p-4 border rounded-md inline-block"><img src={viewingCodesFor.barcode_image} alt="Barcode" className="h-16" /></div>) : (<p>Not generated</p>)}
              ...
              <div>
                ...
                {viewingCodesFor.qrcode_image ? (<div className="bg-white p-2 border rounded-md inline-block"><img src={viewingCodesFor.qrcode_image} alt="QR Code" className="h-40 w-40" /></div>) : (<p>Not generated</p>)}
              </div>
            </div>
            <div className="mt-8 text-center">
              <button onClick={() => setViewingCodesFor(null)} className="bg-blue-600 text-white font-bold px-6 py-2 rounded-lg hover:bg-blue-700">Close</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 print:grid-cols-3 print:gap-1">
            {equipmentList.map((item, index) => (
              <div key={index} className="border border-gray-400 rounded-lg p-2 text-center text-xs break-inside-avoid flex flex-col items-center justify-around aspect-video">
                ...
                {item.barcode_image && <img src={item.barcode_image} alt="Barcode" className="mx-auto h-10 w-full object-contain my-1" />}
                {item.qrcode_image && (<div className="flex items-center gap-2"><p className='font-semibold'>Status:</p><img src={item.qrcode_image} alt="QR Code" className="mx-auto w-16 h-16" /></div>)}
