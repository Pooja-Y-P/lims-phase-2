...
import { api, ENDPOINTS } from '../../api/config'; // Adjust path based on your folder structure
import { ExportMasterStandardPage } from './ExportMasterStandardPage'; // Adjust path based on your folder structure
import { 
  ...
  Search, Loader2, CheckCircle, PowerOff, Eye, Edit, Trash2, X, Save,
...
export const MasterStandardModule: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<MasterStandard | ManufacturerSpec | null>(null);
  ...
  const menuCards: MenuCard[] = [
    ...
    { 
      ...
      colorClass: "bg-emerald-600", desc: "View and edit OEM specs and limits", viewId: "manufacturer-specs-list"
    },
    { 
      id: 3, title: "Interpolation Ranges", icon: <ArrowRightLeft size={24} strokeWidth={2} />,
      colorClass: "bg-purple-600", desc: "Configure range interpolation logic", viewId: "interpolation"
    },
    { 
      id: 4, title: "UNCERTAINTY", icon: <Activity size={24} strokeWidth={2} />,
      colorClass: "bg-orange-500", desc: "Calculate general measurement uncertainty", viewId: "uncertainty"
    },
    { 
      id: 5, title: "Uncertainty of Pressure Gauge", sub: "(Un-PG)", icon: <Gauge size={24} strokeWidth={2} />,
      colorClass: "bg-cyan-500", desc: "Specific pressure gauge uncertainty metrics", viewId: "uncertainty-pg"
    },
    { 
      id: 6, title: "Coverage Factor (k)", icon: <Sigma size={24} strokeWidth={2} />,
      colorClass: "bg-rose-600", desc: "Define expansion coefficients and confidence", viewId: "coverage-factor"
    }
  ];

  const handleCardClick = (viewId: string) => {
    // Only allow master standard and manufacturer specs access for Hydraulic Torque Wrench
    if (viewId === 'master-standard-list' || viewId === 'manufacturer-specs-list') {
      if (selectedCalibration !== 'Hydraulic Torque Wrench') {
        alert('This feature is only available for Hydraulic Torque Wrench equipment type.');
        return;
      }
    }
  ...
  const handleEditItem = (item: MasterStandard | ManufacturerSpec, formViewId: string) => {
  ...
  return (
    <div className="max-w-7xl mx-auto animate-fadeIn">
      ...
      {/* --- VIEW: MASTER STANDARD EXPORT --- */}
      {currentView === 'master-standard-export' && (
        <ExportMasterStandardPage onBack={() => setCurrentView('master-standard-list')} />
      )}
      ...
      {currentView === 'master-standard-form' && (
        <MasterStandardForm 
          ...
          initialData={selectedItem as MasterStandard | null}
        />
      )}

      {/* --- VIEW: MANUFACTURER SPECS LIST --- */}
      ...
      {/* --- VIEW: MANUFACTURER SPECS FORM --- */}
      {currentView === 'manufacturer-specs-form' && (
        <ManufacturerSpecForm 
          onBack={() => setCurrentView('manufacturer-specs-list')}
          initialData={selectedItem as ManufacturerSpec | null}
        />
      )}
      
      {/* Placeholder for unimplemented views */}
      {!['grid', 'master-standard-list', 'master-standard-form', 'master-standard-export', 'manufacturer-specs-list', 'manufacturer-specs-form'].includes(currentView) && (
          <div className="animate-fadeIn">
            <button onClick={handleBackToGrid} className="mb-6 flex items-center text-gray-500 hover:text-blue-600 transition-colors font-medium text-sm">
