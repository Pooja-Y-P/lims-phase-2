...
const DriveInterfaceSection: React.FC<DriveInterfaceSectionProps> = ({ jobId }) => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setLoading(true);
      try {
        const res = await api.get<any>(`${ENDPOINTS.HTW_CALCULATIONS.DRIVE_INTERFACE}/${jobId}`);
        if (res.data.status === "success" && res.data.positions.length > 0) {
            const mappedData = res.data.positions.map((p: any) => ({
                position_deg: p.position_deg,
                readings: p.readings.map(String),
                mean_value: p.mean_value
            }));
            const sorted = [0, 90, 180, 270].map(deg => mappedData.find((d: any) => d.position_deg === deg) || { position_deg: deg, readings: Array(10).fill(""), mean_value: null });
            setTableData(sorted);
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        const res = await api.post<any>(ENDPOINTS.HTW_CALCULATIONS.DRIVE_INTERFACE_CALCULATE, payload);
        setMeta({ set_torque: res.data.set_torque, error_value: res.data.error_value, torque_unit: res.data.torque_unit || "-" });
        setIsSaved(true);
        alert("Section D Saved!");
    ...
    <div className="flex flex-col w-full animate-in fade-in duration-500 bg-white border border-gray-200 rounded-xl shadow-sm p-4 mt-6">
        ...
        <div className="flex justify-between items-center mt-4">
             ...
             {isSaved ? (
                ...
                <button onClick={handleSave} disabled={!isFormComplete || calculating} className={`px-4 py-2 text-xs font-bold uppercase rounded-lg flex gap-2 shadow-sm transition-all ${isFormComplete ? "bg-indigo-600 text-white hover:bg-indigo-700" : "bg-gray-200 text-gray-400"}`}>{calculating ? <Loader2 className="h-4 w-4 animate-spin"/> : <Save className="h-4 w-4"/>} Save</button>
             )}
        </div>
    </div>
  );
